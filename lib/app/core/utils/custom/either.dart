abstract class CustomEither<TFailure, TSuccess> {
  ...
  static SuccessResponse<TFailure, TSuccess> success<TFailure, TSuccess>(TSuccess value) => SuccessResponse<TFailure, TSuccess>(value);

  static FailureResponse<TFailure, TSuccess> failure<TFailure, TSuccess>(TFailure value) => FailureResponse<TFailure, TSuccess>(value);

  bool get isSuccess => _success != null;
  bool get isFailure => _failure != null;

  // ignore: unnecessary_this
  TSuccess? operator |(TSuccess? newFailureResponse) => this.fold(onFailure: (_) => newFailureResponse, onSuccess: (value) => value);
}
...
class SuccessResponse<TFailure, TSuccess> extends CustomEither<TFailure, TSuccess> {
  SuccessResponse(TSuccess value) : super(success: value);
}

class FailureResponse<TFailure, TSuccess> extends CustomEither<TFailure, TSuccess> {
  FailureResponse(TFailure value) : super(failure: value);
}

class ResponseNotDefinedFailure extends EitherFailure {
  ResponseNotDefinedFailure() : super(message: 'Invalid Response. Failure or Success not defined');
}
